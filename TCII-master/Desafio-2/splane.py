...
def analyze_sys( all_sys, aprox_name,ideal_filter):
...
#    img_ext = 'svg'
    ...
    for ii in range(cant_sys):
        fig_id, axes_hdl = bodePlot(all_sys[ii], fig_id, axes_hdl,ideal_filter=ideal_filter[ii])
...
#    plt.show()
    ...
    for ii in range(cant_sys):
        fig_id, axes_hdl = grpDelay(all_sys[ii], fig_id,ideal_filter=ideal_filter[ii])
...
def grpDelay(myFilter, fig_id='none',ideal_filter=[]):
    ...
    plt.axvline(w_d, color='r', ls="dotted")
...
def bodePlot(myFilter, fig_id='none', axes_hdl='none', label = '',ideal_filter=[]):
    ...
    plt.sca(mag_ax_hdl)
    alfa_max=-ideal_filter[0]
    alfa_min=-ideal_filter[1]
    w_p=ideal_filter[2]
    w_s=ideal_filter[3]
    alfamax=[]
    alfamin=[]
    wp=[]
    ws=[]
    #if(ideal_filter[5]!="Bessel"):
    if(ideal_filter[4]=="Pasa_Bajo"):
    
        for valor in w:
              if(valor<w_p):
                  wp.append(valor)       
                  alfamax.append(alfa_max)
              elif (valor>w_s):
                  ws.append(valor)       
                  alfamin.append(alfa_min)
    elif (ideal_filter[4]=="Pasa_Alto"):
        w_s=1/w_s
    ...
    plt.semilogx(ws,alfamin,color='r', ls="dotted")
    plt.semilogx(wp,alfamax,color='r', ls="dotted")
    
    yp=np.where(w < w_p)[0][-1]
    plt.annotate(str(mag[yp].round(2)) + "dB@" + str(w_p) +"r/s",xy=(w_p, mag[yp]),xycoords='data',xytext=(3, 0.4),fontsize=9,arrowprops=dict(arrowstyle="->",connectionstyle="arc3,rad=.2"))
    ys=np.where(w>w_s)[0][0]
    plt.annotate(str(mag[ys].round(2)) + "dB@"+ str(w_s)+"r/s",xy=(w_s, mag[ys]),xycoords='data',xytext=(w_s, -10),fontsize=9,arrowprops=dict(arrowstyle="->",connectionstyle="arc3,rad=.2"))
    plt.axvline(w_s, color='r', ls="dotted")
    plt.axvline(w_p, color='r', ls="dotted")
    plt.ylim([alfa_max+10,alfa_min-10])    
